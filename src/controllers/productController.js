...
const getProducts = async (req, res) => {
  try {
    ...
    res.json({ message: "Error al obtener productos", error });
  }
};
...
const getProductById = async (req, res) => {
  try {
    ...
    res.json(200).json(result.rows[0]);
  } catch (error) {
...
const createProduct = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO products (id, name, description, price, image_url, stock) VALUES ($1, $2, $3, $4, $5,$6) RETURNING *",
