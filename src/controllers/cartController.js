...
const updateCartItem = async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length === 0) return res.status(404).json({ message: "Producto no encontrado en el carrito" });
...
const deleteCartItem = async (req, res) => {
    try {
        const result = await pool.query("DELETE FROM cart WHERE id = $1 AND user_id = $2 RETURNING *", [
            req.params.id,
            req.user.userId,
        ]);
